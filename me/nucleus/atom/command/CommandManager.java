...
import me.nucleus.atom.command.commands.BindCommand;
...
public class CommandManager
{
    ...
    public CommandManager() {
        ...
        AtomMod.log.info("Commands initialised");
    }
    ...
    public void callCommand(final String command) {
        ...
        for (final Command c : this.commands) {
            ...
            for (int j = 0; j < c.getAliases().size(); ++j) {
                if (c.getAliases().get(j).equalsIgnoreCase(label)) {
                    c.call(parts);
                    return;
                }
            }
